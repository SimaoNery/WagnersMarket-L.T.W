...
try {
    ...
    if (isset($_POST['csrf'])) {
        ...
        if ($csrf === $session->getToken()) {
            ...
            if (Wishlist::removeFromWishlist($db, $session->getId(), $itemId)) {
                Item::decrementWishlistCounter($db, $itemId);
                $response = ['success' => 'Item successfully removed from your wishlist.'];
            } else {
                $response = ['error' => 'An error occurred! Couldn\'t remove the item to from your wishlist.'];
            }
