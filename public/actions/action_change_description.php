    ...
    if (isset($_POST['csrf'])) {
        ...
        if ($csrf === $session->getToken()) {
            ...
            if (Item::getItem($db, $itemId)) {
                if (Item::changeDescription($db, $itemId, $brand)) {
                    $session->addMessage('success', 'Description changed successfully.');
                } else {
                    $session->addMessage('error', 'Failed to change Description.');
                }
            } else {
